...
def direct_root_connect(Vertex_processed, Edges_raw):
	for i in  range(len(Vertex_processed)):
		...
		for j in range(len(Edges_raw)):
			if Edges_raw[j][0]>Edges_raw[j][1] and Edges_raw[j][0]==(Vertex_processed[Edges_raw[j][0]]).old_parent:
				(Vertex_processed[Edges_raw[j][0]]).parent = min((Vertex_processed[Edges_raw[j][0]]).parent, Edges_raw[j][1])
			elif(Edges_raw[j][1]==(Vertex_processed[Edges_raw[j][1]]).old_parent):
				(Vertex_processed[Edges_raw[j][1]]).parent= min((Vertex_processed[Edges_raw[j][1]]).parent, Edges_raw[j][0])
...
AlgorithmRA(Vertex_raw, Edges_raw)